<template>
 <view class="cmd-bottom-nav" :class="[textAuto ? 'cmd-bottom-nav-text-auto':'',fixed ? 'cmd-bottom-nav-fixed':'']" :style="setColorStyle">
<!--  <view :class="['cmd-bottom-nav-box',select == index ? 'cmd-bottom-nav-active':'']" :style="select == index ? setActiveFontColorStyle : ''"
	  ...
	  <view :class="['cmd-bottom-nav-box', 'cmd-bottom-nav-active']" :style="select == index ? setActiveFontColorStyle : ''"
	    @tap="$_click(index)" v-for="(nav,index) in list" :key="index">
		 <view class="cmd-bottom-nav-box-icon">
			<cmd-icon v-if="nav.icon && !nav.src" :type="nav.icon" size="18" :color="select == index ?activeFontColor : fontColor"></cmd-icon>
		  </view>
