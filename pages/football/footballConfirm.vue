<template>
	<div class="football-confirm padding-bottom-90 box">
		...
		<u-popup :show="show" @close="show=false" style="z-index: 0;" mode="bottom" closeable>
			<span style="padding: 10px 20px;">自由过关(多选)</span>
			<view class="tag">
				<uni-row>
			...
			<span v-if="mnStrandArr.length>0" style="padding: 10px 20px;">M串N(单选)</span>
			<view v-if="mnStrandArr.length>0" style="margin-bottom: 130px;" class="tag">
				<uni-row>
					<uni-col :span="5" v-for="(item, index) in mnStrandArr" :key="index">
						<u-tag :text="item.name" :plain="!item.checked" type="warning" :name="index"
							@click="checkboxClickMN">
						</u-tag>
					</uni-col>
				</uni-row>
			</view>
			<!-- 占位 -->
			<view  v-if="mnStrandArr.length==0" style="margin-bottom: 130px;"></view>
		...
		data() {
			return {
				...
				mnTable:{
					...
					'2': [],  // 3场以下没有m串n
					'3': [
						{name: '3串3', index: 10, checked: false, typeList:[2]},
						{name: '3串4', index: 11, checked: false, typeList:[2, 3]},
					],
					'4': [
						{name: '4串4', index: 12, checked: false, typeList:[3]},
						{name: '4串5', index: 13, checked: false, typeList:[3, 4]},
						{name: '4串6', index: 14, checked: false, typeList:[2]},
						{name: '4串11', index: 15, checked: false, typeList:[2, 3, 4]},
					],
					'5': [
						{name: '5串5', index: 16, checked: false, typeList:[4]},
						{name: '5串6', index: 17, checked: false, typeList:[4, 5]},
						{name: '5串10', index: 18, checked: false, typeList:[2]},
						{name: '5串16', index: 19, checked: false, typeList:[3, 4, 5]},
						{name: '5串20', index: 20, checked: false, typeList:[2, 3]},
						{name: '5串26', index: 21, checked: false, typeList:[2, 3, 4, 5]},
					],
					'6': [
						{name: '6串6', index: 22, checked: false, typeList:[5]},
						{name: '6串7', index: 23, checked: false, typeList:[5, 6]},
						{name: '6串15', index: 24, checked: false, typeList:[2]},
						{name: '6串20', index: 25, checked: false, typeList:[3]},
						{name: '6串22', index: 26, checked: false, typeList:[4, 5, 6]},
						{name: '6串35', index: 27, checked: false, typeList:[2, 3]},
						{name: '6串42', index: 28, checked: false, typeList:[3, 4, 5, 6]},
						{name: '6串50', index: 29, checked: false, typeList:[2, 3, 4]},
						{name: '6串57', index: 30, checked: false, typeList:[2, 3, 4, 5, 6]},
					],
					'7': [
						{name: '7串7', index: 31, checked: false, typeList:[6]},
						{name: '7串8', index: 32, checked: false, typeList:[6, 7]},
						{name: '7串21', index: 33, checked: false, typeList:[5]},
						{name: '7串35', index: 34, checked: false, typeList:[4]},
						{name: '7串120', index: 35, checked: false, typeList:[2, 3, 4, 5, 6, 7]},
					],
					'8': [
						{name: '8串8', index: 36, checked: false, typeList:[7]},
						{name: '8串9', index: 37, checked: false, typeList:[7, 8]},
						{name: '8串28', index: 38, checked: false, typeList:[6]},
						{name: '8串56', index: 39, checked: false, typeList:[5]},
						{name: '8串70', index: 40, checked: false, typeList:[4]},
						{name: '8串247', index: 41, checked: false, typeList:[2, 3, 4, 5, 6, 7, 8]},
					],
				}
			}
		},
		...
		methods: {
			...
			init(itemList, flag) {
				...
				if (itemList.length > 0) {
					...
					this.strandArr = arr;
					this.mnStrandArr = this.mnTable[itemList.length.toString()];
					// 选中玩法
			...
			checkboxClick(name) {
				// 清除M串N
				for(let n=0; n < this.mnStrandArr.length; n++){
					this.mnStrandArr[n].checked = false;
				}
				console.log('标签', this.strandArr[name]);
				console.log('标签选择事件',name);
				this.strandArr[name].checked = !this.strandArr[name].checked
				//将选中的值赋值给下拉框
				this.text = ""
				this.calculationParam.pssTypeList = [] // 投注方式，2为2串1，3为3串1.....8为8串1
				...
				console.log(JSON.stringify(this.calculationParam.pssTypeList));
			...
			//M串N标签选择事件
			checkboxClickMN(name) {
				// M串N为单选，清除所有N串1的选中状态
				for(let n=0; n < this.strandArr.length; n++){
					this.strandArr[n].checked = false;
				}
				// 清除其他已选中的M串N
				for(let n=0; n < this.mnStrandArr.length; n++){
					if(name!=n){
				...
				this.mnStrandArr[name].checked = true;
				// this.mnStrandArr[name].checked = !this.mnStrandArr[name].checked
				// 将选中的值赋值给下拉框
				this.text = ""
				// this.calculationParam.pssTypeList = [] // 投注方式，2为2串1，3为3串1.....8为8串1
				this.calculationParam.pssTypeList = this.mnStrandArr[name].typeList;
				this.mnStrandArr.map(map => {
					if (map.checked) {
						this.text += map.name
						// this.calculationParam.pssTypeList.push(map.index)
					}
				})
				console.log(JSON.stringify(this.calculationParam));
				if (this.calculationParam.pssTypeList.length > 0) {
					this.calculationFootball();
				} else {
					this.price = 0;
